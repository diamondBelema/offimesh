...
Settlement flow:
1. Look up merchant's bank account
2. Initiate transfer via Nomba API (`POST /v2/transfers/bank`)
3. Mark settlement as complete on success
4. Failed settlements retry automatically (Celery task every 5 min, max 3 retries)
5. Webhooks from Nomba update settlement status in real time
...
**No auth required** (HMAC-SHA256 signature verification).

**Headers:**
| Header          | Required | Description                   |
|-----------------|----------|-------------------------------|
| nomba-signature | yes      | HMAC-SHA256 hex signature     |
...
| Event                    | Action                              |
|--------------------------|-------------------------------------|
| `virtual_account.funded` | Credit user wallet, create ledger entry |
| `transfer.successful`    | Mark settlement completed           |
| `transfer.failed`        | Mark settlement failed, trigger retry |
...
Always returns 200 (prevents Nomba retries). Processing is offloaded.
...
| `NOMBA_BASE_URL`                | `https://sandbox.api.nomba.com/v1`   | Nomba API base URL                 |
| `NOMBA_ACCOUNT_ID`              | —                                    | Parent Nomba account ID            |
| `NOMBA_SUBACCOUNT_ID`           | —                                    | Team sub-account ID                |
...
| Nomba calls   | Circuit breaker: 5 failures in 60s → 120s open |
...
1. REGISTER
   POST /v1/auth/register
   └─→ User created, virtual NUBAN created via Nomba, OTP sent via SMS
...
4. FUND WALLET
   ...
   └─→ Nomba webhook credits your wallet automatically
